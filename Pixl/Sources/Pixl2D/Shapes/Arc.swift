...
public struct Arc: Hashable, Sendable {
    ...
    public init(radius: Float, angle: Angle, width: Float) {
    ...
    ///   - angle: Finite aperture strictly within `0...360` degrees.
    ///   - width: Positive full line width.
    public static func arc(radius: Float, angle: Angle, width: Float) -> Self { .init(radius: radius, angle: angle, width: width) }
}
