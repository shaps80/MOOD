struct PositionedLineBuffer: ~Copyable {
    private var storage: UnsafeMutablePointer<PositionedLine>?
    ...
    subscript(index: Int) -> PositionedLine {
        precondition(index >= 0 && index < count)
        return storage![index]
    }

    mutating func append(_ line: consuming PositionedLine) {
        ensureCapacity(for: count + 1)
        storage!.advanced(by: count).initialize(to: line)
        count += 1
    }
    ...
    func withSpan<Result: ~Copyable>(
        _ body: (Span<PositionedLine>) throws -> Result
    ) rethrows -> Result {
    ...
    private mutating func ensureCapacity(for required: Int) {
        ...
        let newStorage = UnsafeMutablePointer<PositionedLine>.allocate(capacity: newCapacity)
