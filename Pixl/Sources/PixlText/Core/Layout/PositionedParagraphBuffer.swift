struct PositionedParagraphBuffer: ~Copyable {
    private var storage: UnsafeMutablePointer<PositionedParagraph>?
    ...
    subscript(index: Int) -> PositionedParagraph {
        precondition(index >= 0 && index < count)
        return storage![index]
    }

    mutating func append(_ paragraph: consuming PositionedParagraph) {
        ensureCapacity(for: count + 1)
        storage!.advanced(by: count).initialize(to: paragraph)
        count += 1
    }
    ...
    func withSpan<Result: ~Copyable>(
        _ body: (Span<PositionedParagraph>) throws -> Result
    ) rethrows -> Result {
    ...
    private mutating func ensureCapacity(for required: Int) {
        ...
        let newStorage = UnsafeMutablePointer<PositionedParagraph>.allocate(capacity: newCapacity)
