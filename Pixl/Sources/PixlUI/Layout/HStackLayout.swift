...
public struct HStackLayout: Layout {
    ...
    public init(alignment: VerticalAlignment = .center, spacing: Float? = nil) { self.alignment = alignment; self.spacing = spacing }
    ...
    public func sizeThatFits(proposal: ProposedViewSize, subviews: Subviews, cache: inout Void) -> CGSize {
        ...
        result.width += Float(max(0, subviews.count - 1)) * (spacing ?? 0)
