import Swift

struct _StateContext: @unchecked Sendable {
    let store: _StateStore
    let path: [_ViewIdentity.Component]
    let viewType: ObjectIdentifier

    func location<Value>(field: String, initialValue: Value) -> _StateLocation<Value> {
        store.location(
            identity: .init(path: path, viewType: viewType, field: field),
            initialValue: initialValue
        )
    }
}

enum _StateRuntime {
    @TaskLocal static var context: _StateContext?
}

struct _ViewIdentity: Hashable, @unchecked Sendable {
    enum Component: Hashable {
        case structural(UInt32)
        case explicit(AnyHashable)
    }

    var path: [Component]

    static let root = Self(path: [.structural(0)])

    func child(_ index: UInt32) -> Self {
        var path = path
        path.append(.structural(index))
        return .init(path: path)
    }

    func explicit<ID: Hashable>(_ id: ID) -> Self {
        var path = path
        path.append(.explicit(AnyHashable(id)))
        return .init(path: path)
    }
}
