...
public struct ProfilerView: View {
    ...
    public var body: some View {
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack {
                ...
                Text(frame.map { "Frame \($0.correlation) · CPU span \(format($0.duration))" } ?? "Waiting for recorded frames")
                    .font(.caption.monospaced())
                Spacer()
                Text("\(format(timeBounds.lowerBound - timeOrigin)) → \(format(timeBounds.upperBound - timeOrigin))").font(.caption.monospaced())
            }
            ...
            .frame(minHeight: 110, maxHeight: expanded ? 600 : 240)
        ...
        return GeometryReader { geometry in
            Canvas { context, size in
                ...
                for (index, value) in values.enumerated() {
                    ...
                    let height = max(2, value.duration / maximum * size.height)
                    let rect = CGRect(x: Double(index) * width, y: size.height - height,
                                      width: max(1, width - 1), height: height)
                    context.fill(Path(rect), with: .color(value.correlation == selected ? .orange : .cyan.opacity(0.6)))
                }
            }
        ...
        return GeometryReader { geometry in
            Canvas { context, size in
                ...
                for segment in segments {
                    ...
                    context.fill(Path(roundedRect: rect, cornerRadius: 2), with: .color(color(segment.scope)))
                    if rect.width > 55 {
    ...
    private func color(_ id: UInt32) -> Color {
        if snapshot.scopes.first(where: { $0.id == id })?.kind == .wait { return .gray }
        let color: Color
        switch id % 7 {
        case 0: color = .cyan
        case 1: color = .mint
