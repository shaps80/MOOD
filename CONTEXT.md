...
Pixl2D collision broad-phase storage follows Box2D's dynamic AABB-tree direction. `DynamicAABBTree2D` owns a manually allocated contiguous node pool with automatic geometric growth, integer parent/child links, free-list reuse, and generational `ProxyID` handles. Greedy surface-area insertion and local perimeter-reducing rotations maintain topology. Nearest-ray queries use parent links for stackless traversal and progressively prune beyond the closest leaf hit. Production tree storage and queries do not use `Array`, collection abstractions, fixed traversal stacks, or steady-state allocation. Tree proxies contain geometric bounds only; collider meaning, discrete narrow-phase contact generation, and later continuous shape sweeps remain separate layers.
