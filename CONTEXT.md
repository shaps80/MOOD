...
`RenderQueue`
: Pixl's public game-owned, long-lived two-dimensional submission domain. A game normally creates each queue once from its `GameContext`, selecting a fixed positive capacity. Submission never grows that storage; Foundation reports overflow and the public API treats exceeding configured capacity as a programmer error. Entities submit world-space values but do not encode them. Camera/view state belongs to execution rather than an individual sprite. One submitted snapshot may therefore be executed for one camera or reused across several camera views, including split-screen, before the queue begins its next submission lifecycle. The exact public execution/reset surface remains a deliberate API gate. Independent queues still serve genuinely independent destinations such as a pixel-art world and native-resolution UI. The unqualified name is intentional while Pixl has no 3D submission API; naming can be reconsidered when one exists.
