...
Pixl2D collision broad-phase storage follows Box2D's dynamic AABB-tree direction. `DynamicAABBTree2D` owns a manually allocated contiguous node pool with automatic geometric growth, integer links, free-list reuse, and generational `ProxyID` handles. Production tree storage and queries do not use `Array`, collection abstractions, or steady-state allocation. Tree proxies contain geometric bounds only; collider meaning, discrete narrow-phase contact generation, and later continuous shape sweeps remain separate layers.
