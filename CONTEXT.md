...
`Polygon2D` is immutable normalized local-space geometry. Construction removes redundant boundary vertices, normalizes winding, rejects invalid simple polygons, computes bounds, and triangulates once. Retained vertices and indices use immutable unsafe buffers; temporary high-level collections are confined to this cold construction path. `Polygon` adds paint and submits that geometry through the shared queue. Queue-local open-addressed unsafe registries resolve repeated geometry identity and polygon batch compatibility without steady-state allocation or linear lookup. Polygon draws reuse shared culling, ordering, and batching, then use indexed geometry with compact per-instance transforms and colours. Solid colour is the initial supported material; gradients temporarily use their first colour and textures temporarily use grey at the explicit lowering fallback.
