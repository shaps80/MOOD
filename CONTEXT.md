...
Shapes support canonical unit sizing or explicit dimensions, modifier-style fill and stroke, per-shape rounding such as `Shape(.rect, rounding: 8)`, and `.antialiasing(.smooth)` or `.antialiasing(.hard)` with `.smooth` as the default. The default style is a white fill, no stroke, normal blending, and layer/order zero. Stroke is centred by default, with inside and outside alignment available; unsupported rounding combinations fail explicitly rather than being ignored. Analytic distance produces fill coverage, stroke/annular coverage, rounding, and antialiasing without texture filtering. Nearest/linear filtering is irrelevant to direct analytic rendering; when shapes are rendered into a low-resolution render texture, the later composite sprite's filtering controls scaling.
...
For steady-state GPU performance, arbitrary gradients compile once into rows of a shared fixed-capacity 2D lookup atlas. A gradient shape then carries compact atlas coordinates and performs one linearly filtered lookup regardless of stop count; different gradients retain one shared texture/sampler binding and can remain batch-compatible. Solid fills use a separate no-sample path. A direct two-colour interpolation may be retained only if measurement beats the atlas on Metal and WebGPU. `RenderSettings.maxGradientCount` defaults to 256 unique resolved gradients; row resolution remains an internal measured choice. Rendering must not traverse, copy, allocate, or hash gradient stop arrays per frame. Transient-shape lifetime diagnostics are not part of the current plan.
