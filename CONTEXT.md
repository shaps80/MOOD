...
Analytic 2D SDF shapes are a separate render family rather than a sprite material or game-facing wrapper around sprites and shapes. The initial source catalogue is the complete fixed and variable-parameter primitive set documented by Inigo Quilez at `https://iquilezles.org/articles/distfunctions2d/`; its public names, parameters, formula reuse terms, and explicit parameter-compatible shader/storage families must be catalogued before implementation. Arbitrary polygon is a notable variable-length outlier requiring a deliberate representation rather than silently inflating every fixed-parameter shape.
...
Shapes support canonical unit sizing or explicit dimensions, modifier-style fill and stroke, per-shape rounding such as `Shape(.rect, rounding: 8)`, and `.antialiasing(.smooth)` or `.antialiasing(.hard)` with `.smooth` as the default. Analytic distance produces fill coverage, stroke/annular coverage, rounding, and antialiasing without texture filtering. Nearest/linear filtering is irrelevant to direct analytic rendering; when shapes are rendered into a low-resolution render texture, the later composite sprite's filtering controls scaling. Orthographic submission interprets dimensions, rounding, and stroke widths as local/world units transformed with the shape. Screen-space submission interprets them as target pixels without requiring a game-facing camera. The exact screen-space submission spelling, default fill/stroke, stroke alignment, unsupported-rounding behaviour, and shape blend properties remain API gates.

`PixlGraphics.Gradient` is planned as dimension-independent retained colour-ramp data closely matching SwiftUI's `Gradient`: `init(colors:)`, `init(stops:)`, nested `Stop(color:location:)`, and public stops. Gradient construction and shape styling are cold-path operations. Linear, radial, angular, or other placement belongs to shape fill styles rather than `Gradient` itself; the initial placement set, normalized coordinate vocabulary, stop validation, interpolation colour space, and hard-stop behaviour remain explicit gates.

For steady-state GPU performance, arbitrary gradients should compile once into rows of a shared fixed-capacity 2D lookup atlas. A gradient shape then carries compact atlas coordinates and performs one linearly filtered lookup regardless of stop count; different gradients retain one shared texture/sampler binding and can remain batch-compatible. Solid fills use a separate no-sample path. A direct two-colour interpolation may be retained only if measurement beats the atlas on Metal and WebGPU. Atlas capacity and row resolution require startup/default decisions; rendering must not traverse, copy, allocate, or hash gradient stop arrays per frame. Transient-shape lifetime diagnostics are not part of the current plan.
