...
Internal `CollisionStore2D` owns a second manually allocated, geometrically growing record pool containing exact bounds, fat broad-phase bounds, stable generational collider identity, tree proxy identity, and static/dynamic state. Tree leaves carry the collider slot directly as opaque `Int32` user data so candidate traversal requires no hash table or reverse lookup. Exact bounds may move within their fat bounds without tree mutation; escaping bounds are reinserted. Contact traversal rejects self and static-static candidates before exact `Rect` narrow phase. Exact ray tests clip broad-phase traversal to the nearest confirmed collider. These are hidden implementation facilities, not an accepted public collision API. Future public filtering vocabulary will use `CollisionLayer`; its precise relationship with the future `CollisionMask` remains subject to explicit API review before implementation.
