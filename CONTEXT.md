...
Game-owned `CollisionWorld2D` is the public Rect-only discrete collision boundary. Games explicitly call `advance()` once per fixed tick after synchronizing changed bounds; Pixl's runtime and `GameContext` neither own nor automatically run collision. Not constructing or advancing a collision world therefore adds no implicit per-tick work. Each advance centrally generates unique physical pairs, rejects static-static pairs, performs exact narrow phase once, and emits directed `.began`, `.changed`, or `.ended` reports through reusable contiguous storage. The callback form of `advance` lets games return corrected world bounds for each report's source, which the collision world immediately synchronizes without exposing a second post-callback update step. `CollisionMask` filtering is intentionally one-way: a source receives a report when its mask contains the target's `CollisionLayer`; the target need not opt in. Reciprocal interest creates two oppositely oriented reports from one exact contact test. Collision endpoints contain opaque `ColliderID` identity and exact world-space bounds captured for that collision tick. Games own entity lookup and callback dispatch.

Internal `CollisionStore2D` owns a second manually allocated, geometrically growing record pool containing exact bounds, fat broad-phase bounds, stable generational collider identity, tree proxy identity, pre-lowered layer/mask bits, and static/dynamic state. A dense unsafe dynamic-index buffer makes advance traversal proportional to live dynamic colliders rather than historical record capacity. Tree leaves carry the collider slot directly as opaque `Int32` user data so candidate traversal requires no hash table or reverse lookup. Exact bounds may move within their fat bounds without tree mutation; escaping bounds are reinserted. Directed contacts are written to geometrically growing unsafe buffers, heap-sorted in place by their generational endpoint tuple, and linearly merged with the prior tick to derive phases without hashes or steady-state allocation. Exact public overlap and ray queries use the tree only as broad phase, filter target layers, and confirm exact Rect results before reporting.
