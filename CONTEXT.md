...
Pixl2D collision broad-phase storage follows Box2D's dynamic AABB-tree direction. `DynamicAABBTree2D` owns a manually allocated contiguous node pool with automatic geometric growth, integer parent/child links, free-list reuse, and generational `ProxyID` handles. Greedy surface-area insertion and local perimeter-reducing rotations maintain topology. Overlap and ray queries use parent links for stackless traversal. Ray queries report broad-phase candidates through a callback; their caller performs the exact shape test and returns a clipping distance so traversal can prune farther nodes. The tree moves or enlarges caller-supplied broad-phase bounds and does not choose fattening policy itself. Production tree storage and queries do not use `Array`, collection abstractions, fixed traversal stacks, or steady-state allocation. Tree proxies contain geometric bounds only; collider meaning, discrete narrow-phase contact generation, fat-AABB policy, and later continuous shape sweeps remain separate layers. Broad-phase types remain implementation API rather than public game-facing API.

Game-owned `CollisionWorld2D` is the public discrete collision boundary for axis-aligned rectangles and transformed `Polygon2D`, `Circle2D`, and `Capsule2D` geometry. Games explicitly call `advance()` once per fixed tick; Pixl's runtime and `GameContext` neither own nor automatically run collision. Not constructing or advancing a collision world therefore adds no implicit per-tick work. Each advance centrally generates unique physical pairs, rejects static-static pairs and pairs where neither mask requests contact, performs exact narrow phase once, and emits directed `.began`, `.changed`, or `.ended` reports through reusable contiguous storage. Polygon narrow phase traverses its baked convex triangles with separating-axis tests while retaining original-boundary metadata so internal decomposition edges can reject overlap but never become reported contact normals. Circle and capsule transforms are lowered into cached world geometry only when inserted or marked dirty; their collision contract permits translation, rotation, reflection, and uniform scale because non-uniform scale or shear would no longer preserve the analytic shape. Exact contact and ray hot paths consume only the cached world geometry. The callback form of `advance` lets games return a corrected `Transform2D` for each report's source. Changes automatically enter a de-duplicated unsafe dirty-index buffer and synchronize once after report delivery; explicit transform updates behave identically. `CollisionMask` filtering is intentionally one-way: a source receives a report when its mask contains the target's `CollisionLayer`; the target need not opt in. Reciprocal interest creates two oppositely oriented reports from one exact contact test. Collision endpoints contain opaque `ColliderID` identity and exact world-space AABB captured for that collision tick. Games own entity lookup and callback dispatch.

Internal `CollisionStore2D` owns a second manually allocated, geometrically growing record pool containing collider geometry, exact bounds, fat broad-phase bounds, stable generational collider identity, tree proxy identity, pre-lowered layer/mask bits, and static/dynamic state. A dense unsafe dynamic-index buffer makes advance traversal proportional to live dynamic colliders rather than historical record capacity. A separate dense dirty-index buffer makes transform synchronization proportional only to colliders changed since the preceding synchronization and coalesces repeated updates. Polygon colliders retain immutable local geometry plus unsafe transformed-vertex, triangle-axis, and boundary-edge buffers; transformed data and AABB are rebuilt only when dirty. Tree leaves carry the collider slot directly as opaque `Int32` user data so candidate traversal requires no hash table or reverse lookup. Exact bounds may move within their fat bounds without tree mutation; escaping bounds are reinserted. Directed contacts are written to geometrically growing unsafe buffers, heap-sorted in place by their generational endpoint tuple, and linearly merged with the prior tick to derive phases without hashes or steady-state allocation. Exact public overlap and ray queries synchronize pending transforms, use the tree only as broad phase, filter target layers, and confirm exact rectangle or polygon geometry before reporting.

`PlatformerController` is a provisional fixed-step gameplay controller currently housed under Pixl's `Prototypes/Platformer` domain so its behavior can be validated before moving to a separate `PixlPrototypes` package. It consumes semantic `PlatformerInput` snapshots, advances deterministic movement state without steady-state allocation, returns world displacement, and accepts directed `Contact2D` values for velocity response and grounded/head/wall classification. It owns movement configuration, state, timers, velocity, facing, and stance; it does not own an entity, collision world, collider identity, bindings, animation, or rendering. The current Game remains the integration consumer. Its character collider is still a temporary AABB; replacing it with `Capsule2D`, applying crouch stance to physical geometry, and using the configured ground/head/wall probe distances remain the next integration step.
...
: A public allocation-free value returned by `GameContext.coordinates(for:)` for input conversion. Resolution performs camera projection dispatch and inversion once; `coordinates(relativeTo:)` may then invert and compose an object's local transform once. `location(for:)` and `translation(for:)` apply only the stored composed transform to typed mouse values, events, or samples. Invalid resolution produces `.invalid` locations and zero translations without stopping execution.
...
RenderQueue is broader than a physical sprite batch. It coordinates sprites, analytic shapes, immediate primitives, and indexed polygons in one authoritative order. Each family retains its own compatible GPU batching and upload path; Foundation shares ordering, culling, storage, cache, and lifetime infrastructure without forcing different vertex layouts or shaders into one draw.
...
Analytic 2D SDF shapes are a separate render family rather than a sprite material or game-facing wrapper around sprites and shapes. The initial source catalogue is every fixed-parameter primitive documented by Inigo Quilez at `https://iquilezles.org/articles/distfunctions2d/`; its public names, parameters, and explicit parameter-compatible shader/storage families must be catalogued before implementation. Arbitrary polygons are variable-length indexed geometry and therefore use their own render family rather than inflating every fixed-parameter shape.

The fixed-parameter catalogue is: circle; rectangle/rounded rectangle; segment; rhombus; trapezoid; parallelogram; equilateral, isosceles, and arbitrary triangles; uneven capsule; pentagon, hexagon, and octagon; hexagram and regular star; pie, cut disk, arc, ring, and horseshoe; vesica and moon; rounded cross, egg, heart, cross, and rounded X; ellipse; parabola and parabola segment; quadratic Bezier; blobby cross; tunnel; stairs; quadratic circle; hyperbola; cool S; and circle wave. Oriented boxes are expressed by ordinary rectangle geometry plus `Transform2D`, avoiding a duplicate authoring concept; formula-local orientation parameters remain only where they change the primitive itself, such as arcs.

`Polygon2D` is immutable normalized local-space geometry. Construction removes redundant boundary vertices, normalizes winding, rejects invalid simple polygons, computes bounds, and triangulates once. Retained vertices and indices use immutable unsafe buffers; temporary high-level collections are confined to this cold construction path. `Polygon` adds paint and submits that geometry through the shared queue. Queue-local open-addressed unsafe registries resolve repeated geometry identity and polygon batch compatibility without steady-state allocation or linear lookup. Polygon draws reuse shared culling, ordering, and batching, then use indexed geometry with compact 48-byte per-instance records. GPU geometry bakes normalized local coordinates once alongside each local vertex. Solid colour, the shared gradient atlas, and stretched texture-region paints share one polygon vertex stage and select small paint-specific fragment stages; textured polygons preserve the asset's alpha representation and existing filtering/addressing semantics.

`Circle2D` and `Capsule2D` are immutable local-space geometry shared by collision and rendering. `Shape(circle)` and `Shape(capsule)` reuse the existing analytic circle and capsule SDF families, including their fill, gradient, stroke, and antialiasing behavior; no additional platform shader path is introduced. The capsule lowering derives its local orientation and extent once per submission from its centre segment.
