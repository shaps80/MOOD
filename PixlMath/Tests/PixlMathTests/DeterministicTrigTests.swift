...
struct DeterministicTrigTests {
    ...
    func normalizedSinCos() {
        for index in 0...65_536 {
            ...
            let magnitudeSquared = result.cosine * result.cosine
                + result.sine * result.sine
    ...
    func canonicalAtan2() {
        ...
        for value in cases {
            #expect(abs(atan2(y: value.y, x: value.x) - value.angle) < 0.000_05)
        }
    }
    ...
    func stableFloatBitPatterns() {
        ...
        for index in 0..<65_536 {
            ...
            hash = mix(direction.bitPattern, into: hash)
        }
