...
- `ShaderCatalogue.default` is PixlGraphics' generated built-in shader and Pixl registers it automatically before `PlatformGame.prepare(on:)`. `Platform.shaders` is the platform-owned `ShaderRegistry`; games use preparation only to append their own shaders without managing shader-library lifetime or platform resource loading.
