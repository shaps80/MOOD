...
- `Game` is the game-facing lifecycle. `GameRuntime<G: Game>` owns the concrete mutable game and platform-neutral `Loop`, implements `PlatformGame`, and is what concrete platform runtimes receive. `Game.didEnter`, `fixedUpdate`, and `update` are mutating so value-type games can own their state directly; `render` remains read-only. Every callback receives the stable `GameContext`, which exposes the platform, drawable format, audio, assets, and mutable `timeScale` without retaining all `RenderSettings`. `GameContext.timeScale` affects simulation delta while `UpdateTime.unscaledDelta` continues during pause. Game initialization still runs after the platform and built-in shaders exist, allowing immutable startup resources without optional storage.
...
- `PixlPlatform.Keyboard` exposes physical `Key` state plus fixed-capacity, double-buffered, contiguous per-frame `Key.Event` transitions. `Key.Phase` is `.down`/`.up`; held state is queried separately with `contains`. Native repeat downs coalesce once per key per frame, modifiers travel as `Key.Modifiers`, `isFocused` reports focus, and focus loss synthesizes releases. macOS translates hardware key codes; WASM translates `KeyboardEvent.code` and focuses the game canvas/document.
- `PixlPlatform.Gamepads` dynamically grows stable controller slots only on connection; frame polling and each `Gamepad`'s button/event storage remain allocation-free and fixed-capacity. Physical button names use face `.north`/`.east`/`.south`/`.west`, D-pad `.up`/`.down`/`.left`/`.right`, shoulders, normalized triggers, stick presses, `.menu`, and `.options`; Home, adaptive-trigger output, and haptics are excluded for now. Stick vectors and all public analogue values use `Double`, are y-up, and impose no platform dead zone. Button transitions are events; stick movement is continuous state and produces no events. macOS retains controllers using connection notifications; WASM consumes the standard Gamepad mapping. Browser privacy hides already-connected controllers until a visible-page controller gesture. Connect/disconnect logging occurs once per state change.
- Continuous keyboard/gamepad state belongs in fixed simulation when appropriate; per-presentation input events must not be consumed independently by every fixed tick because one presentation may run zero or multiple ticks. Games should latch one-shot events into fixed simulation when needed.
...
Build the browser Game target with `.scripts/build`, package it with `.scripts/_package-web`, and serve it with `.scripts/serve`. Use `.scripts/clean` to remove that product's WASM scratch directory when its build cache becomes inconsistent. All default to the sibling `Game` package and product.
