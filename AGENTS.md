...
- `PixlPlatform.AudioEngine` owns fixed-capacity resident sounds, voices, flat buses, portable playback semantics, stable sound replacement, and sound availability. Removal stops matching voices and makes play fail until replacement; valid content changes restart matching voices while preserving playback handles and controls. Native completion handlers only set atomic flags; finished voices are reclaimed on demand without a frame-time reaper.
