...
- `PixlPlatform.AudioEngine` owns fixed-capacity resident sounds, voices, flat buses, portable playback semantics and control values, stable sound replacement, and sound availability. `Audio.play`, `AudioDevice.play`, and backend playback throw `AudioError` and return a nonoptional `Playback`/voice. `Audio`/`AudioDevice` expose labeled get/set subscripts for playback volume, pan, rate, and bus volume plus a `masterVolume` property; absent handles read as zero and writes do nothing. Removal stops matching voices and makes play throw until replacement; valid content changes restart matching voices while preserving playback handles and controls. Native completion handlers only set atomic flags; finished voices are reclaimed on demand without a frame-time reaper.
